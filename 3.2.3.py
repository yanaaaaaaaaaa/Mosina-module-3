...
class DataSet:
    ...
    def get_stats_without_cities(self, vacancy_name):
        ...
        salary_stat = {k: salary_stat[k] // vacancy_count_stat[k] for k in sorted(salary_stat)}
        selected_salary_stat = {k: selected_salary_stat[k] // selected_count_stat[k] if selected_count_stat[k] != 0 else 0 for k in sorted(selected_salary_stat)}
        return {'vacancy_count_stat': vacancy_count_stat, 'salary_stat': salary_stat, 'selected_salary_stat': selected_salary_stat, 'selected_count_stat': selected_count_stat}
...
def do(tuple):
    d = tuple[0]
    vacancy_name = tuple[1]
    return d.get_stats_without_cities(vacancy_name)


def feature(files, vacancy_name):
    with ftr.ProcessPoolExecutor(8) as exc:
        data_sets = exc.map(read_csv, files)

    with ftr.ProcessPoolExecutor(8) as exc:
        results = exc.map(do, [(i, vacancy_name) for i in data_sets])

    return results

#def multi(files, vacancy_name):
#    with Pool(8) as pool1:
#        data_sets = pool1.map(read_csv, files)
#
#    with Pool(8) as pool2:
#        results = pool2.map(do, [(i, vacancy_name) for i in data_sets])
#
#    return results
...
def union(results):
    ...
    selected_count = {}
    for d in results:
        salary = salary | d['salary_stat']
        count = count | d['vacancy_count_stat']
        selected_count = selected_count | d['selected_count_stat']
        selected_salary = selected_salary | d['selected_salary_stat']

    return salary, count, selected_salary, selected_count
...
if __name__ == '__main__':
    ...
    results = feature(files, vacancy_name)
    salary, count, selected_salary, selected_count = union(results)

    #results = read_csv(path).get_stats_without_cities(vacancy_name)
    #salary = results['salary_stat']
    #count = results['vacancy_count_stat']
    #selected_salary = results['selected_salary_stat']
    #selected_count = results['selected_count_stat']
